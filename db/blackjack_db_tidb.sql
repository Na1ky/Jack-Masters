-- TiDB Compatible SQL Dump
-- Aggiungiamo i DROP per sicurezza nel caso alcune tabelle siano state create a metà
...
CREATE TABLE `levels` (
  `Lvl` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE `games` (
  `IdGame` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `Date` date NOT NULL,
  CONSTRAINT `User` FOREIGN KEY (`Username`) REFERENCES `users` (`Username`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `players` (
  ...
  CONSTRAINT `Level` FOREIGN KEY (`Lvl`) REFERENCES `levels` (`Lvl`),
